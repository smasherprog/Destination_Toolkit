...
void MY_UI_Too::Init(MY_UI_Too::Interfaces::IRenderer* renderer, MY_UI_Too::Interfaces::ISkin* skin, MY_UI_Too::Interfaces::IFont_Factory* font_factory, MY_UI_Too::Interfaces::IWidget* root, unsigned int screen_width, unsigned int screen_height){
	Internal::Renderer=renderer; 
	Internal::UI_Skin = skin; 
	Internal::Root_Widget = root;

	Internal::Renderer->Init();
	Internal::Root_Widget->Set_Control_Bounds(MY_UI_Too::Utilities::Rect(0, 0, screen_width, screen_height));

	Internal::UI_Skin->Init(Internal::Renderer);
	Utilities::SetCursor(Utilities::Standard);

	Internal::Font_Factory = font_factory;
	Internal::Font_Factory->Init();
	Internal::Font_Factory->Get_Font("Times New Roman");// to build the default font
