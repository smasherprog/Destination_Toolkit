...
namespace MY_UI_Too{
	class DirectX11 : public MY_UI_Too::IRenderer
	{
	public:
		...
		virtual void Begin(MY_UI_Too::Utilities::ITexture* texture=nullptr) override;
		...
		virtual void OnResize(unsigned int x, unsigned int y) override { Inv_Wndx = (1.0f/static_cast<float>(x))*2.0f; Inv_Wndy = (1.0f/static_cast<float>(y))*-2.0f; }
		virtual unsigned int GetDrawCalls()const override{ return DrawCalls;}

	protected:

		void Draw(MyDrawState& drawstate);
		bool SetTexture(MY_UI_Too::Utilities::ITexture* pTexture, bool getnewbuffer);
