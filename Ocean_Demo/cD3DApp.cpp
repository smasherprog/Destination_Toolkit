...
D3DApp::D3DApp(HINSTANCE hInstance, std::string appname, unsigned int height, unsigned int width) : cBaseD3D(hInstance, appname, height, width) {
	...
	Ocean_Demo = new Ocean();
	Ocean_Demo->Init(&sundir, &UICamera->Camera->Position, &UICamera->Camera->VP,UICamera->Camera->Planes, worldsize, gridsize);    

	//popular the ocean with some grids
	for(float i = gridsize*-4.0f; i != gridsize*4.0f; i+=gridsize){
		for(float j= gridsize*-4.0f; j != gridsize*4.0f; j+=gridsize){
			vec3 tlpos(i, 0, j);
			if(Ocean_Demo->AddGrid(tlpos)){
				Ocean_Demo->GridAdded(tlpos);
			}
		}
	}
	...
	// create the ocean
	UIOcean = new UI_Ocean();
	UIOcean->Init(Ocean_Demo);

	// add a sunspeed control to the ocean window
	MY_UI::Controls::HorizontalSlider_w_Label<float>* vsliderlab3 = new MY_UI::Controls::HorizontalSlider_w_Label<float>(UIOcean->window);
	...
	std::vector<MY_UI::Controls::cWidgetBase*>& child =UIOcean->window->Internal_Widget.Children;
...
void D3DApp::run(){
	while(cBaseD3D::begin()){// while we should keep running
		...
		FrameTimer.Per_Loop();
		UICamera->Camera->PerFrame(FrameTimer.DT);// update camera
