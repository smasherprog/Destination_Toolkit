#include "stdafx.h"
#include "Simple_Mesh_Shader.h"
#include <sstream>

Simple_Mesh_Shader::Simple_Mesh_Shader(){
	Quality =0 ;
	Matrix_Count= 60;
	Init();
}
...
void Simple_Mesh_Shader::Init(){
	Compile();
}
void Simple_Mesh_Shader::DeInit(){
	VS.Destroy();
	PS.Destroy();
}

void Simple_Mesh_Shader::Compile(){
