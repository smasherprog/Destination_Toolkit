struct VSParticleIn
{
    ...
    float  Timer            : BLENDWEIGHT0;            //timer for the particle
    float  Type             : BLENDWEIGHT1;             //particle type
};
...
cbuffer cb0
{
	...
	float4x4 g_invview;
    float4 g_fGlobalTime;
    float4 g_fElapsedTime;
	float4 g_vFrameGravity;
};

#define g_fSecondsPerFirework 1
#define g_iNumEmber1s 50
#define g_fMaxEmber2s 15.0
...
{
    float tCoord = (g_fGlobalTime.x + fOffset) / 300.0;
    return g_txRandom.SampleLevel( sampler1, tCoord, 0 );
}
...
{
    input.pos += input.vel*g_fElapsedTime.x;
    input.vel += g_vFrameGravity.x;
    input.Timer -= g_fElapsedTime.x;
...
void GSLauncherHandler( VSParticleIn input, inout PointStream<VSParticleIn> ParticleOutputStream )
{
    if(input.Timer <= 0)
    {
        ...
        VSParticleIn output;
        output.pos = input.pos + input.vel*g_fElapsedTime.x;
...
void GSScenemain(point VSParticleDrawOut input[1], inout TriangleStream<PSSceneIn> SpriteStream)
{
    ...
    for(int i=0; i<4; i++)
    {
        float3 position = g_positions[i]*input[0].radius+ input[0].pos;

		output.pos = mul(float4(position, 1), g_mWorldViewProj);
...
{   
    return g_txDiffuse.Sample( sampler1, input.tex ) * input.color;
}
