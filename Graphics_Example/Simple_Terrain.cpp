...
void Simple_Terrain::Init(){
	OUTPUT_DEBUG_MSG("Calling Simple_Terrain::Init()");
	EditorSize=64;
	SelectedTex=1;
	EditorFunction=0;
	...
	InitGrids();
	HS.CompileShaderFromFile("Simple_Terrain.fx");
	DS.CompileShaderFromFile("Simple_Terrain.fx");
	DS_Editor.CompileShaderFromFile("Simple_Terrain.fx", "DSEditor");
	PS.CompileShaderFromFile("Simple_Terrain.fx");
	CBuffer0.Create(1, sizeof(buff0), CONSTANT_BUFFER);
	CBuffer1.Create(1, sizeof(vec4), CONSTANT_BUFFER);
	StartingPos.zero();	
	Editing=DrawWireFrame=false;
	EditorPos.zero();
	OUTPUT_DEBUG_MSG("Finished Simple_Terrain::Init()");
}

void Simple_Terrain::DeInit(){	
	...
	OUTPUT_DEBUG_MSG("Finished Simple_Terrain::DeInit()");
	Base_Terrain::DeInit();
	DeInitTempGrid();
	DeInitTextures();
	DeInitGrids();
...
void Simple_Terrain::Update(){
	...
	if(update) Threading::RunTask(std::bind(&Simple_Terrain::BeginBuildingNext, this));
}
void Simple_Terrain::Draw(){
...
void Simple_Terrain::DeInitGrids(){
	Points.Destroy();
}

void Simple_Terrain::InitGrids(){
...
void Simple_Terrain::DeInitTempGrid(){
	Verticies.resize(0);
	TexCoords.resize(0);
}

void Simple_Terrain::InitTempGrid(){
...
void Simple_Terrain::DeInitTextures(){
	Diffuse.Destroy();
	Normals.Destroy();
...
void Simple_Terrain::InitTextures(){
...
void Simple_Terrain::BeginBuildingNext(){
